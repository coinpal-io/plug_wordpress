...
// 			}
			...
			/*if ($posted['PGW_CURRENCY'] == $posted['BIL_CURRENCY'] && $order->total != $posted['PGW_PRICE']) {
				$comment = "order:".$order->id." grandTotal=".$order->total." , no equal to PGW_PRICE=:".$posted['PGW_PRICE'];
				WC_Gateway_coinpal::log($comment);
				die ($comment);
			}*/
			
			WC_Gateway_coinpal::log( "Found order #" .$order->id." s:".$REQ_INVOICE."  notify:".json_encode($posted));
			
			$this->validate_amount_currency( $order, $posted );

//			if (!$this->iscoinpal($order->payment_method)) {
//				WC_Gateway_coinpal::log("#" . $order->id." payment method changed");
...
//			}
	...
	public function validate_notify() {
		...
		if($valid){
			WC_Gateway_coinpal::log( "Received valid response from coinpal , notify:".json_encode($_POST));
			return true;
		} else {
			WC_Gateway_coinpal::log( "Received invalid response from coinpal, notify:".json_encode($_POST) );
		}
	...
	private function payment_status_paid( $order, $posted ) {
		if ( $order->has_status( "completed" ) ) {
			WC_Gateway_coinpal::log( "Aborting, Order #" . $order->id . " is already complete." );
			exit;
		}
	...
	private function payment_status_cancelled( $order, $posted ) {
		...
		$order->update_status( "canceled", sprintf( __( "Payment %s via coinpal Notify.", "woocommerce" ), wc_clean( $posted["status"] ) ) );
	}
	...
	private function payment_status_failed( $order, $posted ) {
		$order->update_status( "failed", sprintf( __( "Payment %s via coinpal Notify.", "woocommerce" ), wc_clean( $posted["status"] ) ) );
	}
